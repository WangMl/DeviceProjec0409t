...
public class ForumAction extends BaseAction<Forum> {
	...
	public String list() throws Exception {
		...
		/**
		 * 每次查询四条数据，分别放到不同的map集合里
		 */
		int count = forumService.caculateCount();
		if(count%4==0){
		    countPer =count/4;
		}else{
			countPer =count/4+1;
		...
		for(int i=0;i<countPer;i++){		
			List<Forum> forumList = forumService.findPerFour(4*i,4);
			ActionContext.getContext().put("forumList"+i, forumList);
		}
		ActionContext.getContext().getValueStack().push(countPer);
		return "list";
	}
