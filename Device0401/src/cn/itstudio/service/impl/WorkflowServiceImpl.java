...
public class WorkflowServiceImpl extends DaoSupportImpl<WorkflowBean> implements IWorkflowService {
	/**请假申请Dao*/
	...
	public void saveStartProcess(WorkflowBean workflowBean,Long deviceBillId) {
		//1：获取请假单ID，使用请假单ID，查询请假单的对象DeviceBill
		DeviceBill deviceBill = deviceBillService.getById(deviceBillId);
		//2：更新请假单的请假状态从0变成1（初始录入-->审核中）
		deviceBill.setState(1);
		//3：使用当前对象获取到流程定义的key（对象的名称就是流程定义的key）
		String key = deviceBill.getClass().getSimpleName();
		/**
		...
		/**
		 * 5：	(1)使用流程变量设置字符串（格式：DeviceBill.id的形式），通过设置，让启动的流程（流程实例）关联业务
   				(2)使用正在执行对象表中的一个字段BUSINESS_KEY（Activiti提供的一个字段），让启动的流程（流程实例）关联业务
		 */
		//格式：DeviceBill.id的形式（使用流程变量）
		String objId = key+"."+deviceBillId;
	...
	public DeviceBill findDeviceBillByTaskId(String taskId) {
		//1：使用任务ID，查询任务对象Task
		...
		//5：获取BUSINESS_KEY对应的主键ID，使用主键ID，查询请假单对象（DeviceBill.1）
		String id = "";
		...
		//查询请假单对象
		//使用hql语句：from DeviceBill o where o.id=1
		DeviceBill deviceBill = deviceBillService.getById(Long.parseLong(id));
		return deviceBill;
	}
	...
	public void saveSubmitTask(WorkflowBean workflowBean) {
		...
		//在流程定义结束之前查询DeviceBill对象，否则无法查询
		DeviceBill bill = findDeviceBillByTaskId(taskId);
		/**
...
//		}
	...
	public List<Comment> findCommentByDeviceBillId(Long id) {
		//使用请假单ID，查询请假单对象
		DeviceBill deviceBill = deviceBillService.getById(id);
		//获取对象的名称
		String objectName = deviceBill.getClass().getSimpleName();
		//组织流程表中的字段中的值
